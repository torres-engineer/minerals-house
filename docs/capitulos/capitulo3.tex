...
Este capítulo descreve como o ``Casa dos Minerais'' foi implementado na prática. O jogo corre tanto nativamente como no browser, o que obrigou a separar a lógica de simulação da camada de apresentação.
...
O projeto usa dois tipos de estruturas de dados: estáticas e compactas em Odin para o estado do jogo, e dinâmicas em JSON/JavaScript para o conteúdo descritivo.
...
A estrutura \texttt{GameState} centraliza toda a informação mutável, contendo o jogador e o registo de itens encontrados. A posição e o destino são vetores de 2 elementos (`[2]f32`), permitindo cálculos matemáticos diretos.
...
A Figura~\ref{fig:decomposicao-layers} mostra como as diferentes camadas se sobrepõem para compor o cenário.
...
As perguntas são geradas proceduralmente com base no conteúdo JSON carregado. O sistema escolhe aleatoriamente um eletrodoméstico ou mineral encontrado e apresenta quatro opções de resposta. O estado de progresso (quantas perguntas acertou) determina se o nível seguinte é desbloqueado. Esta lógica reside inteiramente no JavaScript, o que simplifica a integração com a interface HTML modal onde o utilizador seleciona as respostas.
